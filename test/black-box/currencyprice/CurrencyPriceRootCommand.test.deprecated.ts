...
describe('CurrencyPriceRootCommand', () => {
    ...
    test('Should get one new CurrencyPrice', async () => {
        const res = await testUtil.rpc(currencyPriceCommand, ['VCL', 'INR']);
        res.expectJson();
    ...
    test('Should fail to get CurrencyPrice without from currency as VCL', async () => {
        const res = await testUtil.rpc(currencyPriceCommand, ['INR', 'EUR']);
        res.expectJson();
        res.expectStatusCode(404);
        expect(res.error.error.message).toBe('fromCurrency must be VCL, but was INR.');
    });
    ...
    test('Should fail to get CurrencyPrice because invalid from currency', async () => {
        ...
        res.expectJson();
        res.expectStatusCode(404);
        expect(res.error.error.message).toBe('fromCurrency must be VCL, but was EUR.');
    });

    test('Should fail to get CurrencyPrice because unsupported currencies', async () => {
        const res = await testUtil.rpc(currencyPriceCommand, ['VCL', 'INR', 'USD', 'TEST']);
        res.expectJson();
        res.expectStatusCode(404);
